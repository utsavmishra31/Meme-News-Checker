...
function App() {
  ...
  const sendTextToAI = async (extractedText) => {
    setAiLoading(true);
    try {
      const res = await fetch('https://meme-backend-eng9.onrender.com/api/analyze-text', {
        method: 'POST',
  ...
  const handleCheckFact = async () => {
    if (image) {
      ...
      setResult('');
      try {
        const res = await fetch('https://meme-backend-eng9.onrender.com/api/extract-from-url', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
